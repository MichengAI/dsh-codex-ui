...
export function registerSettingsPage(ctx: Context): void {
  ...
  ctx.slots.inject('settings.trigger', () => ctx.slots.register({ name: 'settings.trigger', locale: NS },
  ...
  ctx.slots.inject('settings.close', () => ctx.slots.register({ name: 'settings.close', locale: NS }, () => t('settings.back')))
