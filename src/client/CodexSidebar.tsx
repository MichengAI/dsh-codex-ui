...
export type CodexSidebarProps =
  ...
  & PropsRenderSlots<'sidebar.workspaces' | 'sidebar.settings' | 'sidebar.footer.action' | 'sidebar.channels'>
...
export function CodexSidebar({ collapsed, width, openSession, startSession, toggleSidebar, renderSlot, t, useSessions, useWorkspaces }: CodexSidebarProps) {
  ...
  const [imTab, setImTab] = useState<'tasks' | 'channels'>('tasks')
  ...
  const searchEntries = useMemo<SearchEntry[]>(() => {
    ...
    const sessionEntries = sessions.ids
      ...
      .filter((session): session is NonNullable<typeof session> => session !== undefined && session.origin !== 'subagent' && session.origin !== 'im' && !session.blank && !archived.has(session.id))
  ...
  if (!wide) return <aside className="dcu-root dcu-compact" aria-label={t('sidebar.label')}><style>{stylesheet}</style><button type="button" className="dcu-icon" aria-label={t('sidebar.expand')} onClick={toggleSidebar}><IconPanelLeftOutline16 size={16} /></button><footer className="dcu-foot"><div className="dcu-footer-actions">{renderSlot('sidebar.footer.action', { wide: false })}</div><div ref={settingsSeat} className="dcu-settings-seat">{renderSlot('sidebar.settings', { wide: false })}</div></footer></aside>

  return <aside className={collapsed ? 'dcu-root dcu-fading' : 'dcu-root'} style={collapsed ? { width: lastWideWidth.current } : undefined} aria-label={t('sidebar.label')}>
    ...
    <div className="dcu-workspaces">
      <div className="dcu-im-tabs">
        <button type="button" className="dcu-im-tab" data-on={imTab === 'tasks'} onClick={() => { setImTab('tasks') }}>任务</button>
