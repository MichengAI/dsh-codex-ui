...
.dcu-root *{box-sizing:border-box}.dcu-head{height:58px;padding:12px 10px 8px;display:flex;align-items:center;gap:6px}.dcu-brand{border:0;background:transparent;color:inherit;padding:0;display:flex;align-items:center;min-width:0;flex:1}.dcu-brand svg{max-width:100%;height:20px}
...
.dcu-menu{padding:0 10px 10px;display:grid;gap:2px}.dcu-menu button,.dcu-footer-link{display:grid;grid-template-columns:20px minmax(0,1fr);column-gap:8px;align-items:center;width:100%;min-height:36px;padding:0 4px;border-radius:8px;text-align:left;font-weight:550}
...
export function CodexSidebar({ collapsed, openSession, startSession, toggleSidebar, renderSlot, t, useSessions, useWorkspaces }: CodexSidebarProps) {
  ...
  const searchEntries = useMemo<SearchEntry[]>(() => {
    ...
    const settingEntries: SearchEntry[] = [
      ...
      { id: 'settings:skills', group: 'settings', label: t('sidebar.skills'), keywords: t('search.settings'), run: () => { closeSearch(); selectSection(t('sidebar.skills')) } },
  ...
  return <aside className="dcu-root" aria-label={t('sidebar.label')}>
    ...
    <header className="dcu-head"><button type="button" className="dcu-brand" aria-label={t('sidebar.newTask')} onClick={() => { startSession() }}><BrandWordmark size={20} /></button><button type="button" className="dcu-icon" aria-label={t('sidebar.search')} onClick={() => { setSearchOpen(true) }}><IconSearchOutline16 size={20} /></button></header>
