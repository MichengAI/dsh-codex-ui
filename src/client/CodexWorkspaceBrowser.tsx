  ...
  return <section className="dcu-wb" aria-label={t('workspace.label')}>
    ...
    <div className="dcu-wb-tree" role="tree">
      <section className="dcu-wb-section" aria-label={t('workspace.pinned')} data-pin-over={pinDragActive && pinSlot === 'header'} onDragOver={(event) => { if (!pinDragActive && !isSidebarItemDrag(event.dataTransfer)) return; event.preventDefault(); event.dataTransfer.dropEffect = 'move'; if (event.target === event.currentTarget || (event.target instanceof Element && event.target.closest('.dcu-wb-section-head') !== null)) setPinSlot('header') }} onDrop={(event) => { event.preventDefault(); const draggedSession = readSessionDrag(event.dataTransfer, sessionDrag?.sessionId); const draggedWorkspace = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const slot = pinSlot; setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setSessionDrag(undefined); setSessionDropId(undefined); setPinSlot(undefined); if (draggedSession !== undefined) { pinSessionAt(draggedSession, slot === 'header' ? pinSectionSessions[0] : slot === 'end' || slot === undefined || !pinSectionSessions.includes(slot) ? undefined : slot); return } if (draggedWorkspace !== undefined) pinWorkspaceAt(draggedWorkspace, slot === 'header' ? (pinnedGroups[0] === undefined ? undefined : String(pinnedGroups[0].workspaceId)) : slot === 'end' || slot === undefined || pinSectionSessions.includes(slot) ? undefined : slot) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setPinSlot(undefined) }}>
        ...
        <div className="dcu-wb-section-body" data-open={sectionOpen('pinned')}><div>{pinSectionSessions.length === 0 && pinnedGroups.length === 0 && <div className="dcu-wb-empty">{t('workspace.pinnedEmpty')}</div>}{pinSectionSessions.map(id => {
          const session = sessions.byId[id as SessionId]
          if (session === undefined) return null
