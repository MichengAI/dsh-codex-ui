...
import { collapseProjectSessionWindow, expandedForCurrentSession, expandedForSessionMove, moveBefore, nextProjectSessionWindow, orderByIds, pinnedHeaderDropIndicator, projectFolderPresentation, projectSessionWindow, readSessionDrag, readWorkspaceDrag, readWorkspaceGroupDrag, reorderDropBeforeId, resolvePinnedSectionDrop, sessionDropAction, sessionDropAfterHovered, ungroupedSessionIds, visibleSessionIds, workspaceGroupMoveTargets, writeSessionDrag, writeWorkspaceDrag, writeWorkspaceGroupDrag, type ProjectSessionWindowState } from './workspace-browser.ts'
...
.dcu-wb-session-more button{height:28px;border:0;border-radius:8px;padding:0 8px;background:transparent;color:var(--dcu-sidebar-tertiary);font:13px/20px var(--dcu-font,inherit);cursor:pointer}
  ...
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    return <div className={`dcu-wb-project${isPinnedHeaderDrop || dropsBefore ? ' dcu-wb-drop' : ''}${dropsAfterLast ? ' dcu-wb-drop dcu-wb-drop-after' : ''}${projectSessionDrop ? ' dcu-wb-session-drop' : ''}${projectSessionMoveDrop ? ' dcu-wb-session-move-drop' : ''}`} key={workspace.workspaceId} onDragOver={handleProjectDragOver} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setWorkspaceDropTarget(undefined); setSessionDropTarget(undefined) }} onDrop={handleProjectDrop}>
      ...
      {renderedIds.map((id) => {
        ...
        const dropsAfterLastSession = sessionDropTarget?.workspaceId === workspaceId && sessionDropTarget.ontoProject !== true && sessionDropAfterHovered(renderedIds, id, sessionDropTarget.beforeId)
