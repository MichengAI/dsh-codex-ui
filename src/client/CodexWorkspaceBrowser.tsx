...
function CodexWorkspaceTree({ wide, useSessions, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, openPath, openSession, renameSession, renameWorkspace, startSession }: CodexWorkspaceBrowserProps) {
  ...
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    return <div className={`dcu-wb-project${isCurrentWorkspace ? ' dcu-wb-project-current' : ''}`} key={workspace.workspaceId} onDragOver={(event) => { if (workspaceDragId === undefined && !isSidebarItemDrag(event.dataTransfer) && !(zone === 'pinned' && sessionDrag !== undefined)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const after = event.clientY > event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2; const id = String(workspace.workspaceId); const ids = (zone === 'pinned' ? pinnedGroups : regularGroups).map(item => String(item.workspaceId)); const last = ids.length > 0 && ids[ids.length - 1] === id; if (zone === 'pinned' && after && last) { setPinSlot('end'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) } else { setPinSlot(zone === 'pinned' ? id : undefined); setWorkspaceDropId(id); setWorkspaceDropAfter(after) } }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const droppedSession = readSessionDrag(event.dataTransfer, sessionDrag?.sessionId); if (zone === 'pinned' && droppedSession !== undefined) { setSessionDrag(undefined); setSessionDropId(undefined); setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined); pinSessionAt(droppedSession); return } const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const after = event.clientY > event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2; setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined); if (dragged === undefined) return; if (zone === 'pinned') { const ids = pinnedGroups.map(item => String(item.workspaceId)); pinWorkspaceAt(dragged, dropBeforeId(ids, String(workspace.workspaceId), after)); return } setPinnedWorkspaceIds(ids => ids.filter(id => id !== dragged)); const ids = regularGroups.map(item => String(item.workspaceId)); const beforeId = dropBeforeId(ids, String(workspace.workspaceId), after); const hostIds = groups.items.map(item => String(item.workspaceId)); if (moveBefore(hostIds, dragged, beforeId).join() !== hostIds.join()) void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId, beforeId as WorkspaceId | undefined)) }}>
      ...
      {isExpanded && shownIds.length === 0 && <div className="dcu-wb-nochat">{t('workspace.noChat')}</div>}
