...
type WorkspaceDropTarget = { zone: 'pinned' | 'projects'; beforeId?: string }
type SessionDropTarget = { workspaceId: string; beforeId?: string }
...
function CodexWorkspaceTree({ wide, useSessions, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, openPath, openSession, renameSession, renameWorkspace, startSession }: CodexWorkspaceBrowserProps) {
  ...
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    const dropsAfterLast = workspaceDropTarget?.zone === zone && workspaceDropTarget.beforeId === undefined && zoneIds[zoneIds.length - 1] === workspaceId
    return <div className={`dcu-wb-project${isCurrentWorkspace ? ' dcu-wb-project-current' : ''}${isPinnedHeaderDrop || dropsBefore ? ' dcu-wb-drop' : ''}${dropsAfterLast ? ' dcu-wb-drop dcu-wb-drop-after' : ''}`} key={workspace.workspaceId} onDragOver={(event) => { if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const rect = event.currentTarget.getBoundingClientRect(); const after = event.clientY > rect.top + rect.height / 2; const beforeId = reorderDropBeforeId(zoneIds, workspaceDragId, workspaceId, after); setWorkspaceDropTarget(beforeId === null ? undefined : { zone, beforeId }) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setWorkspaceDropTarget(undefined) }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const rect = event.currentTarget.getBoundingClientRect(); const after = event.clientY > rect.top + rect.height / 2; setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged === undefined) return; const beforeId = reorderDropBeforeId(zoneIds, dragged, workspaceId, after); if (beforeId === null) return; if (zone === 'pinned') { pinWorkspaceAt(dragged, beforeId); return } setPinnedWorkspaceIds(ids => ids.filter(id => id !== dragged)); const hostIds = groups.items.map(item => String(item.workspaceId)); if (moveBefore(hostIds, dragged, beforeId).some((id, index) => id !== hostIds[index])) void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId, beforeId as WorkspaceId | undefined)) }}>
      <div className={`dcu-wb-project-head${menuOpen ? ' dcu-wb-menu-open' : ''}${workspaceDragId === workspaceId ? ' dcu-wb-dragging' : ''}`} role="treeitem" aria-expanded={isExpanded} tabIndex={0} draggable onDragStart={(event) => { event.stopPropagation(); setSessionDrag(undefined); setSessionDropTarget(undefined); writeWorkspaceDrag(event.dataTransfer, workspaceId, workspace.title); const preview = document.createElement('div'); preview.className = 'dcu-wb-drag-ghost'; preview.textContent = workspace.title; document.body.appendChild(preview); event.dataTransfer.setDragImage(preview, 16, 18); window.requestAnimationFrame(() => { preview.remove() }); setWorkspaceDragId(workspaceId) }} onDragEnd={() => { setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined) }} onClick={() => { toggleGroup(expandKey, true) }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); dismissTip(); setMenu({ id: workspace.workspaceId, type: 'workspace', x: event.clientX, y: event.clientY }) }} onMouseEnter={(event) => { const box = hoverCardAnchor(event.currentTarget.getBoundingClientRect()); showTip({ kind: 'workspace', id: workspace.workspaceId, title: workspace.title, path: workspace.path, count: workspace.visibleIds.length, left: box.left, top: box.top }) }} onMouseLeave={hideTip} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); toggleGroup(expandKey, true) } }}>
