...
import { Button, Menu, Modal, writeClipboard, type MenuEntry } from '@deepseek-ai/dsh-client-ui-primitives'
import {
...
import { collapseProjectSessionWindow, expandedForCurrentSession, expandedForSessionMove, moveBefore, nextProjectSessionWindow, orderByIds, pinnedHeaderDropIndicator, projectFolderPresentation, projectSessionWindow, readSessionDrag, readWorkspaceDrag, readWorkspaceGroupDrag, reorderDropBeforeId, resolvePinnedSectionDrop, sessionDropAction, sessionDropAfterRowId, ungroupedSessionIds, visibleSessionIds, workspaceGroupMoveTargets, writeSessionDrag, writeWorkspaceDrag, writeWorkspaceGroupDrag, type ProjectSessionWindowState } from './workspace-browser.ts'
...
type SessionDropTarget = { workspaceId: string; beforeId?: string; hoveredId?: string; ontoProject?: boolean; crossWorkspace?: boolean }
...
function CreateGroupIcon() {
  return <IconProjectAddOutlineMedium size={16} />
}
...
function CodexWorkspaceTree({ wide, useSessions, useSessionPendingInteraction, useSessionStatus, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, moveSession, openPath, openSession, renameSession, renameWorkspace, startSession, canDeleteSession, panelActive = false }: CodexWorkspaceBrowserProps) {
  ...
  const [sessionWindows, setSessionWindows] = useState<Record<string, ProjectSessionWindowState>>({})
  const revealMoreSessions = (workspaceId: string) => {
    setSessionWindows(current => ({ ...current, [workspaceId]: nextProjectSessionWindow(current[workspaceId]) }))
  }
  ...
  const toggleProject = (expandKey: string, id: string): void => {
    const open = expanded[expandKey] ?? true
    if (open) setSessionWindows(current => ({ ...current, [id]: collapseProjectSessionWindow() }))
  ...
  const projectMenu = (workspace: (typeof groups.items)[number]): MenuEntry[] => {
    ...
    return [
      ...
      { id: 'rename', label: t('workspace.rename'), icon: <IconEditOutlineMedium size={16} /> },
      { type: 'separator', id: 'project-main-separator' },
      { id: 'moveToGroup', label: t('workspace.moveToGroup'), icon: <IconFolderCloseMedium size={16} />, disabled: moveTargets.length === 0, submenu: moveTargets },
      { id: 'openPath', label: t('workspace.openPath'), icon: <IconFolderOpenOutlineMedium size={16} /> },
      { type: 'separator', id: 'project-archive-separator' },
      { id: 'archiveWorkspace', label: t('workspace.archive'), icon: <IconArchiveOutlineMedium size={16} />, disabled: workspace.visibleIds.length === 0 },
      { type: 'separator', id: 'project-delete-separator' },
      { id: 'delete', label: t('workspace.delete'), icon: <IconTrashOutlineMedium size={16} />, danger: true },
    ]
  }
  ...
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    const workspaceId = String(workspace.workspaceId)
    const sessionWindow = projectSessionWindow(shownIds, sessionWindows[workspaceId] ?? { expanded: false, page: 1 }, currentSessionId !== undefined && shownIds.includes(currentSessionId) ? currentSessionId : undefined)
    const renderedIds = sessionWindow.ids
    const dropAfterSessionId = sessionDropTarget?.workspaceId === workspaceId && sessionDropTarget.ontoProject !== true
      ? sessionDropAfterRowId(sessionDropTarget, renderedIds)
      : undefined
      ...
      <div className={`dcu-wb-collection-head${workspaceGroupDragId === groupId ? ' dcu-wb-dragging' : ''}`} draggable onDragStart={(event) => { event.stopPropagation(); setSessionDrag(undefined); setSessionDropTarget(undefined); setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); setWorkspaceGroupDropTarget(undefined); writeWorkspaceGroupDrag(event.dataTransfer, groupId, group.title); setDragPreview(event.dataTransfer, group.title); setWorkspaceGroupDragId(groupId) }} onDragEnd={() => { setWorkspaceGroupDragId(undefined); setWorkspaceGroupDropTarget(undefined) }}>
        ...
        <span className="dcu-wb-collection-actions" onClick={event => { event.stopPropagation() }} onDragStart={event => { event.preventDefault(); event.stopPropagation() }}>
          <Menu open={groupMenuId === groupId} onClose={() => { setGroupMenuId(undefined) }} items={[
            { id: 'rename', label: t('workspace.renameGroup'), icon: <IconEditOutlineMedium size={16} /> },
            { type: 'separator', id: 'group-delete-separator' },
            { id: 'delete', label: t('workspace.deleteGroupAction'), icon: <IconTrashOutlineMedium size={16} />, danger: true },
          ]} onSelect={action => {
