...
import {
  ...
  SESSION_SECTION_PINS_STORAGE_KEY,
...
import { dropBeforeId, isSidebarItemDrag, moveBefore, orderByIds, readSessionDrag, readWorkspaceDrag, standalonePinnedSessionIds, ungroupedSessionIds, visibleSessionIds, workspaceIdsHiddenByPinnedSessions, writeSessionDrag, writeWorkspaceDrag } from './workspace-browser.ts'
...
function CodexWorkspaceTree({ wide, useSessions, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, openPath, openSession, renameSession, renameWorkspace, startSession }: CodexWorkspaceBrowserProps) {
  ...
  const [sectionSessionIds, setSectionSessionIds] = useState(() => readSessionIds(storage(), SESSION_SECTION_PINS_STORAGE_KEY))
  ...
  const [workspaceDropAfter, setWorkspaceDropAfter] = useState(false)
  ...
  useEffect(() => { writeSessionIds(storage(), SESSION_SECTION_PINS_STORAGE_KEY, sectionSessionIds) }, [sectionSessionIds])
  useEffect(() => { if (sectionSessionIds.length === 0) return; setPinnedWorkspaceIds(ids => ids.length === 0 ? ids : []) }, [sectionSessionIds])
  ...
  const projectPinned = (id: WorkspaceId | string): boolean => pinnedWorkspaceIds.includes(String(id)) && !hiddenPinnedWorkspaceIds.has(String(id))
  ...
  const pinDragActive = workspaceDragId !== undefined || sessionDrag !== undefined
  const assignedIds = workspaces.items.flatMap(workspace => workspace.sessionIds.map(id => String(id)))
  const recentIds = ungroupedSessionIds(sessions.ids ?? Object.keys(sessions.byId), sessions.byId, assignedIds, workspaces.archivedSessionIds).filter(id => !pinSectionSessions.includes(id)).sort((left, right) => (sessions.byId[right as SessionId]?.updatedAt ?? 0) - (sessions.byId[left as SessionId]?.updatedAt ?? 0))
  ...
  const pinSessionAt = (id: string, beforeId?: string): void => { setSectionSessionIds(ids => insertSessionId(ids, id, beforeId)); setPinnedWorkspaceIds(ids => ids.length === 0 ? ids : []) }
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    return <div className={`dcu-wb-project${isCurrentWorkspace ? ' dcu-wb-project-current' : ''}`} key={workspace.workspaceId} onDragOver={(event) => { if (workspaceDragId === undefined && !isSidebarItemDrag(event.dataTransfer) && !(zone === 'pinned' && sessionDrag !== undefined)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const after = event.clientY > event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2; const id = String(workspace.workspaceId); const ids = (zone === 'pinned' ? pinnedGroups : regularGroups).map(item => String(item.workspaceId)); const last = ids.length > 0 && ids[ids.length - 1] === id; if (zone === 'pinned' && after && last) { setPinSlot('end'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) } else { setPinSlot(zone === 'pinned' ? id : undefined); setWorkspaceDropId(id); setWorkspaceDropAfter(after) } }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const droppedSession = readSessionDrag(event.dataTransfer, sessionDrag?.sessionId); if (zone === 'pinned' && droppedSession !== undefined) { setSessionDrag(undefined); setSessionDropId(undefined); setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined); pinSessionAt(droppedSession); return } const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const after = event.clientY > event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2; setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined); if (dragged === undefined) return; if (zone === 'pinned') { const ids = pinnedGroups.map(item => String(item.workspaceId)); pinWorkspaceAt(dragged, dropBeforeId(ids, String(workspace.workspaceId), after)); return } setPinnedWorkspaceIds(ids => ids.filter(id => id !== dragged)); const ids = regularGroups.map(item => String(item.workspaceId)); const beforeId = dropBeforeId(ids, String(workspace.workspaceId), after); const hostIds = groups.items.map(item => String(item.workspaceId)); if (moveBefore(hostIds, dragged, beforeId).join() !== hostIds.join()) void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId, beforeId as WorkspaceId | undefined)) }}>
      <div className={`dcu-wb-project-head${menuOpen ? ' dcu-wb-menu-open' : ''}${workspaceDropId === workspace.workspaceId ? (workspaceDropAfter ? ' dcu-wb-drop dcu-wb-drop-after' : ' dcu-wb-drop') : ''}${workspaceDragId === String(workspace.workspaceId) ? ' dcu-wb-dragging' : ''}`} role="treeitem" aria-expanded={isExpanded} tabIndex={0} draggable onDragStart={(event) => { event.stopPropagation(); setSessionDrag(undefined); writeWorkspaceDrag(event.dataTransfer, String(workspace.workspaceId), workspace.title); const preview = document.createElement('div'); preview.className = 'dcu-wb-drag-ghost'; preview.textContent = workspace.title; document.body.appendChild(preview); event.dataTransfer.setDragImage(preview, 16, 18); window.requestAnimationFrame(() => { preview.remove() }); setWorkspaceDragId(workspace.workspaceId) }} onDragEnd={() => { setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined) }} onClick={() => { if (zone !== 'pinned') toggleGroup(expandKey, true) }} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); dismissTip(); setMenu({ id: workspace.workspaceId, type: 'workspace', x: event.clientX, y: event.clientY }) }} onMouseEnter={(event) => { const box = hoverCardAnchor(event.currentTarget.getBoundingClientRect()); showTip({ kind: 'workspace', id: workspace.workspaceId, title: workspace.title, path: workspace.path, count: workspace.visibleIds.length, left: box.left, top: box.top }) }} onMouseLeave={hideTip} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); if (zone !== 'pinned') toggleGroup(expandKey, true) } }}>
        <span className="dcu-wb-folder" onClick={(event) => { event.stopPropagation(); const box = hoverCardAnchor(event.currentTarget.getBoundingClientRect()); if (isShowing('workspace', workspace.workspaceId)) { dismissTip(); return } showTip({ kind: 'workspace', id: workspace.workspaceId, title: workspace.title, path: workspace.path, count: workspace.visibleIds.length, left: box.left, top: box.top }, { immediate: true }) }}>{zone !== 'pinned' && isExpanded ? <IconFolderOpenOutline16 size={16} /> : <IconFolderClose16 size={16} />}</span><span className="dcu-wb-project-title">{workspace.title}</span><span className="dcu-wb-actions"><Menu open={menuOpen} onClose={() => { setMenu(undefined) }} items={projectMenu(workspace)} onSelect={(id) => { if (busy !== undefined) return; if (id === 'new') { startSession(workspace.workspaceId); setMenu(undefined) }; if (id === 'rename') beginRename('workspace', workspace.workspaceId, workspace.title); if (id === 'pin') { setPinnedWorkspaceIds(ids => togglePinnedWorkspace(ids, workspace.workspaceId)); setMenu(undefined) }; if (id === 'openPath') void run('open-path', () => openPath(workspace.path)); if (id === 'delete') { setDeleteTarget({ id: workspace.workspaceId, kind: 'workspace', title: workspace.title }); setError(undefined); setMenu(undefined) } }} portal dense compact getAnchorRect={menuAt === undefined ? undefined : () => pointerMenuRect(menuAt.x, menuAt.y)} anchor={<button type="button" className="dcu-wb-more" aria-label={t('workspace.actions', { name: workspace.title })} onClick={(event) => { event.stopPropagation(); setMenu(current => current?.id === workspace.workspaceId && current.type === 'workspace' ? undefined : { id: workspace.workspaceId, type: 'workspace' }) }}><IconEllipsisOutline16 size={16} /></button>} /></span><span className="dcu-wb-actions"><button type="button" className="dcu-wb-more" aria-label={t('workspace.newSession')} onClick={(event) => { event.stopPropagation(); startSession(workspace.workspaceId) }}><IconPlusOutline16 size={16} /></button></span>
      </div>
      {zone !== 'pinned' && isExpanded && shownIds.length === 0 && <div className="dcu-wb-nochat">{t('workspace.noChat')}</div>}
  ...
  return <section className="dcu-wb" aria-label={t('workspace.label')}>
    ...
    <div className="dcu-wb-tree" role="tree">
      <section className="dcu-wb-section" aria-label={t('workspace.pinned')} onDragOver={(event) => { if (!pinDragActive && !isSidebarItemDrag(event.dataTransfer)) return; event.preventDefault(); event.dataTransfer.dropEffect = 'move'; if (event.target === event.currentTarget || (event.target instanceof Element && event.target.closest('.dcu-wb-section-head') !== null)) setPinSlot('header') }} onDrop={(event) => { event.preventDefault(); const draggedSession = readSessionDrag(event.dataTransfer, sessionDrag?.sessionId); const draggedWorkspace = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const slot = pinSlot; setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setSessionDrag(undefined); setSessionDropId(undefined); setPinSlot(undefined); if (draggedSession !== undefined) { pinSessionAt(draggedSession, slot === 'header' ? pinSectionSessions[0] : slot === 'end' || slot === undefined || !pinSectionSessions.includes(slot) ? undefined : slot); return } if (draggedWorkspace !== undefined) pinWorkspaceAt(draggedWorkspace, slot === 'header' ? (pinnedGroups[0] === undefined ? undefined : String(pinnedGroups[0].workspaceId)) : slot === 'end' || slot === undefined || pinSectionSessions.includes(slot) ? undefined : slot) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setPinSlot(undefined) }}>
        ...
        <div className="dcu-wb-section-body" data-open={sectionOpen('pinned')}><div>{(pinDragActive || pinSlot === 'header') && <div className={`dcu-wb-pin-start${pinSlot === 'header' ? ' dcu-wb-drop' : ''}`} onDragOver={(event) => { if (!pinDragActive && !isSidebarItemDrag(event.dataTransfer)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setPinSlot('header'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) }} />}{pinSectionSessions.length === 0 && pinnedGroups.length === 0 && pinSlot !== 'header' && <div className="dcu-wb-empty">{t('workspace.pinnedEmpty')}</div>}{pinSectionSessions.map(id => {
          const session = sessions.byId[id as SessionId]
          if (session === undefined) return null
          const title = session.displayTitle
          const workspace = groups.items.find(item => item.visibleIds.includes(id))
          const path = session.cwd ?? workspace?.path
          return <SessionRow key={id} id={id} title={title} flat selected={sessions.current === id} menuOpen={menu?.type === 'session' && menu.id === id} pinned unread={unreadSessionIds.includes(id)} running={session.running === true} t={t} menuItems={sessionMenu(id, title, path)} menuPoint={menu?.type === 'session' && menu.id === id && menu.x !== undefined && menu.y !== undefined ? { x: menu.x, y: menu.y } : undefined} draggable dropActive={sessionDropId === id} onDragStart={(event) => { event.stopPropagation(); setWorkspaceDragId(undefined); writeSessionDrag(event.dataTransfer, id, title); const preview = document.createElement('div'); preview.className = 'dcu-wb-drag-ghost'; preview.textContent = title; document.body.appendChild(preview); event.dataTransfer.setDragImage(preview, 16, 18); window.requestAnimationFrame(() => { preview.remove() }); setSessionDrag({ sessionId: id, workspaceId: workspace?.workspaceId ?? '' }) }} onDragEnd={() => { setSessionDrag(undefined); setSessionDropId(undefined); setPinSlot(undefined) }} onDragOver={(event) => { if (sessionDrag === undefined) return; event.preventDefault(); event.stopPropagation(); setSessionDropId(id); setPinSlot(id) }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const draggedSession = readSessionDrag(event.dataTransfer, sessionDrag?.sessionId); const after = event.clientY > event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2; setSessionDrag(undefined); setSessionDropId(undefined); setPinSlot(undefined); if (draggedSession === undefined) return; pinSessionAt(draggedSession, dropBeforeId(pinSectionSessions, id, after)) }} onOpen={() => { setUnreadSessionIds(ids => ids.filter(item => item !== id)); openSession(id as SessionId) }} onMenuChange={(open) => { setMenu(open ? { id, type: 'session' } : undefined) }} onPin={() => { setSectionSessionIds(ids => ids.filter(item => item !== id)) }} onArchive={() => { void run('archive', () => archiveSession(id as SessionId)) }} onHover={(event) => { const box = hoverCardAnchor(event.currentTarget.getBoundingClientRect()); showTip({ kind: 'session', id, title, project: workspace?.title, path, time: formatHoverTime(session.updatedAt), left: box.left, top: box.top }) }} onLeave={hideTip} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); dismissTip(); setMenu({ id, type: 'session', x: event.clientX, y: event.clientY }) }} onSelectAction={(action) => {
            if (busy !== undefined) return
            if (action === 'rename') beginRename('session', id, title)
            if (action === 'pin') { setSectionSessionIds(ids => ids.filter(item => item !== id)); setMenu(undefined) }
            ...
            if (action === 'openPath' && path !== undefined) void run('open-path', () => openPath(path))
        ...
        })}{pinnedGroups.map(workspace => renderGroup(workspace, 'pinned'))}{pinDragActive && <div className={`dcu-wb-pin-end${pinSlot === 'end' ? ' dcu-wb-drop' : ''}`} onDragOver={(event) => { if (!pinDragActive && !isSidebarItemDrag(event.dataTransfer)) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setPinSlot('end'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) }} />}</div></div>
      </section>
      ...
      <section className="dcu-wb-section" aria-label={t('workspace.recent')}>
        ...
        {recentIds.map(id => {
          ...
          return <SessionRow key={id} id={id} title={title} selected={sessions.current === id} menuOpen={menu?.type === 'session' && menu.id === id} pinned={pinned} unread={unread} running={session.running === true} t={t} menuItems={sessionMenu(id, title, session.cwd)} draggable onDragStart={(event) => { event.stopPropagation(); setWorkspaceDragId(undefined); writeSessionDrag(event.dataTransfer, id, title); const preview = document.createElement('div'); preview.className = 'dcu-wb-drag-ghost'; preview.textContent = title; document.body.appendChild(preview); event.dataTransfer.setDragImage(preview, 16, 18); window.requestAnimationFrame(() => { preview.remove() }); setSessionDrag({ sessionId: id, workspaceId: '' }) }} onDragEnd={() => { setSessionDrag(undefined); setSessionDropId(undefined); setPinSlot(undefined) }} menuPoint={menu?.type === 'session' && menu.id === id && menu.x !== undefined && menu.y !== undefined ? { x: menu.x, y: menu.y } : undefined} onOpen={() => { setUnreadSessionIds(ids => ids.filter(item => item !== id)); openSession(id as SessionId) }} onMenuChange={(open) => { setMenu(open ? { id, type: 'session' } : undefined) }} onPin={() => { setPinnedSessionIds(ids => toggleSessionId(ids, id)) }} onArchive={() => { void run('archive', () => archiveSession(id as SessionId)) }} onHover={(event) => { const box = hoverCardAnchor(event.currentTarget.getBoundingClientRect()); showTip({ kind: 'session', id, title, time: formatHoverTime(session.updatedAt), left: box.left, top: box.top }) }} onLeave={hideTip} onContextMenu={(event) => { event.preventDefault(); event.stopPropagation(); dismissTip(); setMenu({ id, type: 'session', x: event.clientX, y: event.clientY }) }} onSelectAction={(action) => {
            if (busy !== undefined) return
            if (action === 'rename') beginRename('session', id, title)
