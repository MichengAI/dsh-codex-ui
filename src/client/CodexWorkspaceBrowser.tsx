...
import { dropBeforeId, moveBefore, orderByIds, pinnedHeaderDropIndicator, readWorkspaceDrag, ungroupedSessionIds, visibleSessionIds, writeSessionDrag, writeWorkspaceDrag } from './workspace-browser.ts'
...
function CodexWorkspaceTree({ wide, useSessions, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, openPath, openSession, renameSession, renameWorkspace, startSession }: CodexWorkspaceBrowserProps) {
  ...
  const pinnedHeaderDrop = pinSlot === 'header' ? pinnedHeaderDropIndicator(pinnedGroups.map(workspace => String(workspace.workspaceId))) : undefined
  ...
  return <section className="dcu-wb" aria-label={t('workspace.label')}>
    ...
    <div className="dcu-wb-tree" role="tree">
      <section className="dcu-wb-section" aria-label={t('workspace.pinned')} onDragOver={(event) => { if (!pinDragActive) return; event.preventDefault(); event.dataTransfer.dropEffect = 'move'; if (event.target === event.currentTarget || (event.target instanceof Element && event.target.closest('.dcu-wb-section-head') !== null)) { setPinSlot('header'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) } }} onDrop={(event) => { event.preventDefault(); const draggedWorkspace = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const slot = pinSlot; setWorkspaceDragId(undefined); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false); setPinSlot(undefined); if (draggedWorkspace !== undefined) pinWorkspaceAt(draggedWorkspace, slot === 'header' ? (pinnedGroups[0] === undefined ? undefined : String(pinnedGroups[0].workspaceId)) : slot === 'end' || slot === undefined ? undefined : slot) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setPinSlot(undefined) }}>
        ...
        <div className="dcu-wb-section-body" data-open={sectionOpen('pinned')}><div>{pinDragActive && pinnedGroups.length === 0 && <div className={`dcu-wb-pin-start${pinnedHeaderDrop?.kind === 'empty' ? ' dcu-wb-drop' : ''}`} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setPinSlot('header'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) }} />}{pinnedGroups.length === 0 && pinSlot !== 'header' && <div className="dcu-wb-empty">{t('workspace.pinnedEmpty')}</div>}{pinnedGroups.map(workspace => renderGroup(workspace, 'pinned'))}{pinDragActive && <div className={`dcu-wb-pin-end${pinSlot === 'end' ? ' dcu-wb-drop' : ''}`} onDragOver={(event) => { if (!pinDragActive) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setPinSlot('end'); setWorkspaceDropId(undefined); setWorkspaceDropAfter(false) }} />}</div></div>
      </section>
