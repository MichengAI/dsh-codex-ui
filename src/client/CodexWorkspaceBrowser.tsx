...
import { insertPinnedWorkspace, prunePinnedWorkspaceIds, readHostPinnedWorkspaceIds, readPinnedWorkspaceIds, resolveWorkspacePreferencesHydration, savePinnedWorkspaceIds, togglePinnedWorkspace, writeHostWorkspacePreferences } from './pinned-workspaces.ts'
...
type MenuState = { id: string; type: 'workspace' | 'session' | 'group'; x?: number; y?: number } | undefined
...
function CodexWorkspaceTree({ wide, useSessions, useSessionPendingInteraction, useWorkspaces, t, archiveSession, deleteSession, deleteWorkspace, forkSession, insertSessionBefore, insertWorkspaceBefore, openPath, openSession, renameSession, renameWorkspace, startSession }: CodexWorkspaceBrowserProps) {
  ...
  const [workspaceGroups, setWorkspaceGroupsState] = useState<WorkspaceGroup[]>([])
  ...
  const queuePinnedHostWrite = (ids: readonly string[], groups: readonly WorkspaceGroup[]): void => {
    ...
    const pending = pinnedHostWriteRef.current.then(() => writeHostWorkspacePreferences(snapshot.pinnedWorkspaceIds, snapshot.workspaceGroups))
  ...
  useEffect(() => {
    ...
    void readHostPinnedWorkspaceIds().then(host => {
      ...
      const dirty = pinnedHostDirtyRef.current
        ...
        : undefined
      const hydration = resolveWorkspacePreferencesHydration(local, host, dirty)
  ...
  const renderGroup = (workspace: (typeof groups.items)[number], zone: 'pinned' | 'projects') => {
    ...
    return <div className={`dcu-wb-project${isPinnedHeaderDrop || dropsBefore ? ' dcu-wb-drop' : ''}${dropsAfterLast ? ' dcu-wb-drop dcu-wb-drop-after' : ''}${projectSessionDrop ? ' dcu-wb-session-drop' : ''}`} key={workspace.workspaceId} onDragOver={(event) => { if (workspaceDragId !== undefined) { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const rect = event.currentTarget.getBoundingClientRect(); const after = event.clientY > rect.top + rect.height / 2; const beforeId = reorderDropBeforeId(zoneIds, workspaceDragId, workspaceId, after); setWorkspaceDropTarget(beforeId === null ? undefined : { zone, beforeId }); return } if (sessionDrag === undefined || sessionDrag.workspaceId !== workspaceId) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setSessionDropTarget({ workspaceId, beforeId: shownIds[0], ontoProject: true }) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setWorkspaceDropTarget(undefined); setSessionDropTarget(undefined) }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); if (dragged !== undefined) { const rect = event.currentTarget.getBoundingClientRect(); const after = event.clientY > rect.top + rect.height / 2; setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged === undefined) return; const beforeId = reorderDropBeforeId(zoneIds, dragged, workspaceId, after); if (beforeId === null) return; if (zone === 'pinned') { pinWorkspaceAt(dragged, beforeId); return } const hostIds = groups.items.map(item => String(item.workspaceId)); if (moveBefore(hostIds, dragged, beforeId).some((id, index) => id !== hostIds[index])) void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId, beforeId as WorkspaceId | undefined)); return } const drag = sessionDrag; setSessionDrag(undefined); setSessionDropTarget(undefined); if (drag === undefined || drag.workspaceId !== workspaceId) return; void run('session-order', () => insertSessionBefore(workspace.workspaceId, drag.sessionId as SessionId, shownIds[0] as SessionId | undefined)) }}>
  ...
  const renderWorkspaceCollection = (group: WorkspaceGroup) => {
    ...
    const sourceGroupId = workspaceDragId === undefined ? undefined : workspaceGroups.find(candidate => candidate.workspaceIds.includes(workspaceDragId))?.id
    ...
    return <div className={`dcu-wb-collection${groupOrderDrop ? ' dcu-wb-group-order-drop' : ''}`} key={group.id} onDragOver={(event) => { const draggedGroupId = readWorkspaceGroupDrag(event.dataTransfer, workspaceGroupDragId); if (draggedGroupId !== undefined) { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setWorkspaceDropTarget(undefined); const head = event.currentTarget.querySelector<HTMLElement>(':scope > .dcu-wb-collection-head'); const rect = head?.getBoundingClientRect() ?? event.currentTarget.getBoundingClientRect(); const beforeId = reorderDropBeforeId(workspaceGroupIds, draggedGroupId, groupId, event.clientY > rect.top + rect.height / 2); setWorkspaceGroupDropTarget(beforeId === null ? undefined : { beforeId }); return } if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setWorkspaceGroupDropTarget(undefined); setWorkspaceDropTarget({ zone: 'group', groupId, ontoGroup: true }) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setWorkspaceDropTarget(undefined); setWorkspaceGroupDropTarget(undefined) }} onDrop={(event) => { const draggedGroupId = readWorkspaceGroupDrag(event.dataTransfer, workspaceGroupDragId); if (draggedGroupId !== undefined) { event.preventDefault(); event.stopPropagation(); const head = event.currentTarget.querySelector<HTMLElement>(':scope > .dcu-wb-collection-head'); const rect = head?.getBoundingClientRect() ?? event.currentTarget.getBoundingClientRect(); const beforeId = reorderDropBeforeId(workspaceGroupIds, draggedGroupId, groupId, event.clientY > rect.top + rect.height / 2); setWorkspaceGroupDragId(undefined); setWorkspaceGroupDropTarget(undefined); if (beforeId !== null) setWorkspaceGroups(current => moveWorkspaceGroup(current, draggedGroupId, beforeId)); return } if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged !== undefined) setWorkspaceGroups(current => sourceGroupId === groupId ? moveWorkspaceGroupMember(current, dragged, groupId) : assignWorkspaceToGroup(current, dragged, groupId)) }}>
      ...
      <DisclosureBody className="dcu-wb-collection-body" open={isExpanded}>{members.map((workspace) => {
        ...
        return <div key={workspaceId} data-dcu-group-member className={`dcu-wb-group-member${orderDrop ? ' dcu-wb-drop' : ''}${orderDropAfter ? ' dcu-wb-drop dcu-wb-drop-after' : ''}`} onDragOverCapture={(event) => { if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const rect = event.currentTarget.getBoundingClientRect(); const beforeId = reorderDropBeforeId(memberIds, workspaceDragId, workspaceId, event.clientY > rect.top + rect.height / 2); setWorkspaceDropTarget(beforeId === null ? undefined : { zone: 'group', groupId, beforeId }) }} onDropCapture={(event) => { if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const rect = event.currentTarget.getBoundingClientRect(); const beforeId = dragged === undefined ? null : reorderDropBeforeId(memberIds, dragged, workspaceId, event.clientY > rect.top + rect.height / 2); setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged === undefined || beforeId === null) return; setWorkspaceGroups(current => sourceGroupId === groupId ? moveWorkspaceGroupMember(current, dragged, groupId, beforeId) : placeWorkspaceInGroup(current, dragged, groupId, beforeId)) }}>{renderGroup(workspace, 'projects')}</div>
  ...
  const renderUngroupedCollection = () => {
    ...
    return <div className={`dcu-wb-ungrouped${groupOrderDrop ? ' dcu-wb-group-order-drop' : ''}`} onDragOver={(event) => { const draggedGroupId = readWorkspaceGroupDrag(event.dataTransfer, workspaceGroupDragId); if (draggedGroupId !== undefined) { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setWorkspaceDropTarget(undefined); const lastGroupId = workspaceGroupIds[workspaceGroupIds.length - 1]; const beforeId = lastGroupId === undefined ? null : reorderDropBeforeId(workspaceGroupIds, draggedGroupId, lastGroupId, true); setWorkspaceGroupDropTarget(beforeId === null ? undefined : { beforeId: undefined }); return } if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; setWorkspaceGroupDropTarget(undefined); setWorkspaceDropTarget({ zone: 'ungrouped', ontoSection: true }) }} onDragLeave={(event) => { if (event.currentTarget.contains(event.relatedTarget as Node)) return; setWorkspaceDropTarget(undefined); setWorkspaceGroupDropTarget(undefined) }} onDrop={(event) => { const draggedGroupId = readWorkspaceGroupDrag(event.dataTransfer, workspaceGroupDragId); if (draggedGroupId !== undefined) { event.preventDefault(); event.stopPropagation(); const lastGroupId = workspaceGroupIds[workspaceGroupIds.length - 1]; const beforeId = lastGroupId === undefined ? null : reorderDropBeforeId(workspaceGroupIds, draggedGroupId, lastGroupId, true); setWorkspaceGroupDragId(undefined); setWorkspaceGroupDropTarget(undefined); if (beforeId !== null) setWorkspaceGroups(current => moveWorkspaceGroup(current, draggedGroupId)); return } if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged === undefined) return; setWorkspaceGroups(current => assignWorkspaceToGroup(current, dragged)); void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId)) }}>
      ...
      <DisclosureBody className="dcu-wb-collection-body" open={ungroupedExpanded}>{ungroupedGroups.map((workspace) => {
        ...
        return <div key={workspaceId} data-dcu-ungrouped-member className={`dcu-wb-group-member dcu-wb-ungrouped-member${orderDrop ? ' dcu-wb-drop' : ''}${orderDropAfter ? ' dcu-wb-drop dcu-wb-drop-after' : ''}`} onDragOverCapture={(event) => { if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = 'move'; const rect = event.currentTarget.getBoundingClientRect(); const beforeId = reorderDropBeforeId(ungroupedGroupIds, workspaceDragId, workspaceId, event.clientY > rect.top + rect.height / 2); setWorkspaceDropTarget(beforeId === null ? undefined : { zone: 'ungrouped', beforeId }) }} onDropCapture={(event) => { if (workspaceDragId === undefined) return; event.preventDefault(); event.stopPropagation(); const dragged = readWorkspaceDrag(event.dataTransfer, workspaceDragId); const rect = event.currentTarget.getBoundingClientRect(); const beforeId = dragged === undefined ? null : reorderDropBeforeId(ungroupedGroupIds, dragged, workspaceId, event.clientY > rect.top + rect.height / 2); setWorkspaceDragId(undefined); setWorkspaceDropTarget(undefined); if (dragged === undefined || beforeId === null) return; if (groupedIds.has(dragged)) setWorkspaceGroups(current => assignWorkspaceToGroup(current, dragged)); void run('workspace-order', () => insertWorkspaceBefore(dragged as WorkspaceId, beforeId as WorkspaceId | undefined)) }}>{renderGroup(workspace, 'projects')}</div>
      })}</DisclosureBody>
    </div>
  }
