...
const stylesheet = `
.dcu-turn-navigator{position:fixed;z-index:8;top:50%;left:var(--dcu-turn-left,288px);transform:translateY(-50%);max-height:calc(100vh - 120px);overflow-y:auto;padding:8px 0;scrollbar-width:none}.dcu-turn-navigator::-webkit-scrollbar{display:none}.dcu-turn-list{display:grid;gap:7px;margin:0;padding:0;list-style:none}.dcu-turn-link{display:flex;align-items:center;width:16px;height:12px;overflow:hidden;padding:0;border:0;border-radius:7px;background:transparent;color:var(--dsw-alias-label-secondary);font:12px/16px var(--dsw-font-family);text-align:left;cursor:pointer;transition:width 180ms ease,height 180ms ease,padding 180ms ease,background-color 180ms ease,color 180ms ease}.dcu-turn-link::before{width:14px;height:2px;flex:0 0 14px;border-radius:2px;background:currentcolor;content:''}.dcu-turn-summary{min-width:0;margin-left:8px;overflow:hidden;opacity:0;text-overflow:ellipsis;white-space:nowrap;transition:opacity 140ms ease}.dcu-turn-link:hover,.dcu-turn-link:focus-visible{width:min(280px,calc(100vw - 64px));height:32px;padding:0 12px;background:var(--dsw-alias-button-floating-hover);color:var(--dsw-alias-label-primary);outline:0}.dcu-turn-link:hover .dcu-turn-summary,.dcu-turn-link:focus-visible .dcu-turn-summary{opacity:1}.dcu-turn-link[aria-current=true]{width:20px;color:var(--dsw-alias-label-primary)}.dcu-turn-link[aria-current=true]::before{width:20px;flex-basis:20px;height:2px}.dcu-turn-link:focus-visible{box-shadow:0 0 0 2px var(--dsw-alias-button-info-fill)}@media (prefers-reduced-motion:reduce){.dcu-turn-link,.dcu-turn-summary{transition:none}}@media (max-width:760px){.dcu-turn-navigator{left:4px}.dcu-turn-link:hover,.dcu-turn-link:focus-visible{width:min(220px,calc(100vw - 40px))}}
`
...
export function TurnNavigator({ useSession, t }: TurnNavigatorProps) {
  ...
  useEffect(() => {
    ...
    const update = (): void => {
      const next = Math.round(sidebar.getBoundingClientRect().right) + 4
