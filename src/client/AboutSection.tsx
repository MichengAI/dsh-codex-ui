...
import { IconCheckOutlineMedium, IconDownloadOutlineMedium, IconListPenOutlineMedium, IconLoadingOutlineMedium } from './host-icons.ts'
  ...
  return <section ref={root} className="dcu-about" aria-label={t('about.nav')}>
    ...
    <div className="dcu-about-title-row">
      ...
      <div className="dcu-about-links">
        ...
        <a className="dcu-about-external-link" href="https://github.com/MichengAI/dsh-codex-ui/issues" target="_blank" rel="noreferrer" aria-label={t('about.feedback')}><IconListPenOutlineMedium />{t('about.feedback')}</a>
      </div>
    </div>
    ...
    <div className="dcu-about-dependencies-heading"><h3>{t('about.dependencies')}</h3>{state === 'ready' && actionableCount > 0 && <button className="dcu-about-install dcu-about-update-all" type="button" aria-busy={installing === 'all'} disabled={installing !== undefined} onClick={() => { void updateAll() }}>{installing === 'all' ? <IconLoadingOutlineMedium size={14} /> : <IconDownloadOutlineMedium size={14} />}{installing === 'all' ? t('about.updatingAll') : t('about.updateAll')}</button>}</div>
    ...
    {state === 'loading' ? <div className="dcu-about-message" role="status">{t('about.loading')}</div>
      : state === 'failed' ? <div className="dcu-about-message" data-error="true" role="alert">{t(businessRequestErrorKey(loadFailure) ?? 'about.statusFailed')}</div>
        : <div className="dcu-about-dependencies" aria-busy={installing !== undefined}>{dependencies.map(dependency => <article className="dcu-about-dependency" key={dependency.id}><div className="dcu-about-copy"><div className="dcu-about-name-row"><a className="dcu-about-name dcu-about-project-link" href={PROJECT_HOMEPAGES[dependency.id]} target="_blank" rel="noreferrer">{title(dependency.id)}</a><a className="dcu-about-downloads" href={`https://www.npmjs.com/package/${dependency.packageName}`} target="_blank" rel="noreferrer" title={t('about.downloadsSource')} aria-label={`${dependency.packageName} · ${t('about.totalDownloads')} · ${typeof dependency.totalDownloads === 'number' && Number.isSafeInteger(dependency.totalDownloads) && dependency.totalDownloads >= 0 ? dependency.totalDownloads.toLocaleString() : t('about.downloadsUnavailable')}`}><IconDownloadOutlineMedium size={12} aria-hidden="true" /><span className="dcu-about-downloads-count">{typeof dependency.totalDownloads === 'number' && Number.isSafeInteger(dependency.totalDownloads) && dependency.totalDownloads >= 0 ? dependency.totalDownloads.toLocaleString() : '—'}</span></a></div><div className="dcu-about-package" title={dependency.packageName}><a className="dcu-about-project-link" href={PROJECT_HOMEPAGES[dependency.id]} target="_blank" rel="noreferrer">{dependency.packageName}</a>{dependency.version === undefined ? '' : ` · ${dependency.version}`}{dependency.updateAvailable && dependency.latestVersion !== undefined ? ` → ${dependency.latestVersion}` : ''}</div></div><div className="dcu-about-status" data-installed={dependency.installed} data-update={dependency.updateAvailable}>{dependency.installed && !dependency.updateAvailable && <IconCheckOutlineMedium size={14} />}{dependency.updateAvailable ? t('about.updateAvailable') : dependency.installed ? t('about.installed') : t('about.missing')}</div>{(!dependency.installed || dependency.updateAvailable) && <button className="dcu-about-install" type="button" disabled={installing !== undefined} onClick={() => { void install(dependency.id) }}>{installing === dependency.id ? <IconLoadingOutlineMedium size={14} /> : <IconDownloadOutlineMedium size={14} />}{installing === dependency.id ? t('about.installing') : dependency.updateAvailable ? t('about.update') : t('about.install')}</button>}</article>)}</div>}
    {installing !== undefined && <div className="dcu-about-progress" role="status"><span><IconLoadingOutlineMedium size={14} /></span><code>{progress === undefined ? t('about.progressHint') : progressLabel(progress, t)}</code>{progress?.percent !== null && progress?.percent !== undefined && <span className="dcu-about-progress-pct">{progress.percent}%</span>}<div className="dcu-about-progress-bar"><div className="dcu-about-progress-fill" data-wave={progress?.percent === null || progress?.percent === undefined} style={progress?.percent === null || progress?.percent === undefined ? undefined : { width: `${progress.percent}%` }} /></div></div>}
