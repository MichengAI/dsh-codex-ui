...
function isDependencyStatus(value: unknown): value is DependencyStatus {
  ...
  return (status.id === 'experts' || status.id === 'skills' || status.id === 'archive')
    && typeof status.packageName === 'string' && typeof status.installed === 'boolean'
}
...
export function AboutSection({ t }: { t: TranslateNS<typeof NS> }) {
  ...
  return <section className="dcu-about" aria-label={t('about.nav')}>
    ...
    <ul className="dcu-about-features"><li>{t('about.feature.sidebar')}</li><li>{t('about.feature.workspace')}</li><li>{t('about.feature.conversation')}</li></ul>
    ...
    {state === 'loading' ? <div className="dcu-about-message">{t('about.loading')}</div>
      : state === 'failed' ? <div className="dcu-about-message" data-error="true">{t('about.statusFailed')}</div>
        : <div className="dcu-about-dependencies">{dependencies.map(dependency => <article className="dcu-about-dependency" key={dependency.id}><div className="dcu-about-copy"><div className="dcu-about-name">{title(dependency.id)}</div><div className="dcu-about-package">{dependency.packageName}{dependency.version === undefined ? '' : ` · ${dependency.version}`}</div></div><div className="dcu-about-status" data-installed={dependency.installed}>{dependency.installed && <IconCheckOutline16 size={14} />}{dependency.installed ? t('about.installed') : t('about.missing')}</div>{!dependency.installed && <button className="dcu-about-install" type="button" disabled={installing === dependency.id} onClick={() => { void install(dependency.id) }}>{installing === dependency.id ? <IconLoadingOutline16 size={14} /> : <IconDownloadOutline16 size={14} />}{installing === dependency.id ? t('about.installing') : t('about.install')}</button>}</article>)}</div>}
