...
assert.match(sidebar, /className=\{`dcu-root\$\{visualCompact \? ' dcu-compact' : ''\}`\}/, '收缩动画必须用独立视觉状态隔离工作区树')
...
assert.match(sidebar, /SIDEBAR_COLUMN_TRANSITION_MS = 300/, '展开内容必须等宿主 300ms 列宽动画结束后再显示，避免逐帧重排长列表')
assert.match(sidebar, /dcu-sidebar-expanded-in 140ms cubic-bezier/, '侧栏展开完成后必须使用合成层淡入动画')
