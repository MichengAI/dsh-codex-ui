...
assert.match(sidebar, /style\[data-plugin-css="@deepseek-ai\/dsh-client-ui-chat\/TurnNavigator\.module\.css"\][^}]*\.dcu-turn-navigator\{display:none\}/, '新版 DSH 提供官方轮次导航时必须隐藏旧版自定义刻度')
assert.match(sidebar, /:has\(>\[data-chat-flow\]\)>:first-child>nav\{right:auto!important;left:calc\(12px - \(var\(--dsh-composer-side-clearance\) \+ 16px\)\)!important\}/, '官方轮次导航必须镜像到会话左侧')
assert.match(sidebar, /nav>\[role="tooltip"\]\{right:auto!important;left:calc\(100% \+ 10px\)!important/, '左侧官方导航的预览气泡必须向右展开')
