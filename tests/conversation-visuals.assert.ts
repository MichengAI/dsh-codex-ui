...
assert.match(navigator, /getBoundingClientRect\(\)\.right\) \+ 4/, '轮次导航与侧栏之间只能保留 4px 间距')
assert.match(navigator, /height:32px/, '轮次导航悬停或聚焦后必须显著放大')
assert.match(navigator, /width:16px;height:12px/, '轮次导航轨道本身必须保持紧凑')
assert.match(navigator, /width:14px;height:2px/, '轮次导航默认必须是 Codex 风格的细短刻度')
assert.match(navigator, /width:20px;flex-basis:20px;height:2px/, '当前轮次刻度只能略长于默认刻度')
