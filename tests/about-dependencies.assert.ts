...
assert.deepEqual(MANAGED_DEPENDENCIES.slice(btwIndex - 1).map(dependency => dependency.id), ['schedule', 'btw', 'simplify', 'pua', 'review', 'pet', 'market'], '新插件必须位于宠物之前，宠物倒数第二，插件市场最后')
assert.equal(managedDependency('pua')?.packageName, '@michengai/dsh-pua')
assert.equal(managedDependency('review')?.packageName, '@michengai/dsh-code-review')
assert.equal(zh['about.dependency.pua'], 'PUA')
assert.equal(en['about.dependency.pua'], 'PUA')
assert.equal(zh['about.dependency.review'], '代码审查')
assert.equal(en['about.dependency.review'], 'Code Review')
assert.equal(managedDependency('pet')?.packageName, '@michengai/dsh-codex-pet')
assert.equal(zh['about.dependency.pet'], 'Codex 宠物')
assert.equal(en['about.dependency.pet'], 'Codex Pet')
...
assert.equal(managedDependency('dsh'), undefined, '关于页不再列出或管理 DSH 运行时')
...
assert.match(host, /const autoReload = runtime\.environmentKind === 'desktop' \|\| notifyParent/, '接口必须区分 Desktop 托管重载与独立 Web 父进程重载')
...
assert.match(about, /selfVersion/, 'Codex UI 大标题必须显示当前运行版本')
assert.match(locales, /'about\.viewProject': 'GitHub'/, '关于页必须提供中文 GitHub 项目文案')
assert.match(locales, /'about\.feedback': '问题反馈'/, '关于页必须提供中文问题反馈文案')
assert.equal(locales.match(/'about\.viewProject': 'GitHub'/g)?.length, 2, '关于页中英文 GitHub 项目文案必须保持一致')
assert.match(locales, /'about\.feedback': 'Issues'/, '关于页必须提供英文 Issues 文案')
