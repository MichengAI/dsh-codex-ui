...
assert.match(manager, /\['add', '--config\.minimumReleaseAge=0', `\$\{dependency\.packageName\}@latest`\]/, '用户确认安装时必须通过 dsh plugin add 获取 npm latest')
assert.match(manager, /\['update', '--latest', '--config\.minimumReleaseAge=0', dependency\.packageName\]/, '用户确认更新时必须通过 pnpm update --latest 强制刷新 lockfile')
...
assert.match(manager, /@latest/, '安装与更新必须显式请求 npm latest 标签')
