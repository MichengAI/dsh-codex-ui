...
test('费用 iframe 就绪前不可见且不参与焦点，加载提示不改变承载区域', async () => {
  ...
  try {
    ...
    const target = { dcuUsageRuntimeReady: true, location: { origin: location.origin, pathname: USAGE_FRAME_PATH }, document: childDocument, postMessage: vi.fn(), dcuUsageHost: undefined as UsageFrameBridge | undefined }
    ...
    await act(async () => { target.dcuUsageHost!.dismissed() })
