...
assert.match(sidebar, /sidebar\.assistant/, '侧栏必须显示个人助理占位入口')
...
assert.match(sidebar, /disabled title=\{t\('sidebar\.assistantUnavailable'\)\}[^]*IconAgentPresetOutline16/, '个人助理必须是不可点击占位，并保留 Agent 预设图标')
assert.doesNotMatch(sidebar, /selectSection\(t\('sidebar\.agentPresets'\)\)/, '个人助理不得跳转 Agent 预设设置页')
