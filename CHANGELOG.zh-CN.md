...
- 兼容 DSH 0.1.5-rc.1 / rc.2 的附件草稿与全局面板，修复预填和历史召回，支持紧凑侧栏及返回已有会话，并使用明确的会话图标。
- 菜单保留 Codex 外观：项目、模型和权限使用官方自适应宽度，@ 和指令列表跟随输入框全宽；修复附件间距、右栏按钮位置及左侧轮次预览方向，保留官方导航动画。
- 用户消息气泡、Session 日志下载入口和官方文案翻译使用宿主实现，移除旧覆盖；旧版未翻译的文案保留英文，日志入口保留原始按钮。
