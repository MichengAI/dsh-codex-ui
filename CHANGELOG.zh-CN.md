...
- 设置中的对话显示、权限名称及内置指令说明改用 DSH 官方翻译；旧版缺失翻译时保留原始英文，不影响功能。

- 用户消息气泡恢复 DSH 原版长文本显示方式，取消强制完整展开。

- 项目、模型和权限菜单保留 Codex 外观，恢复 DSH 原版自适应宽度，适应不同名称长度。@ 和指令建议菜单跟随输入框调宽，修复内部列表被固定宽度压窄的问题。

- 修复新版纯附件消息排列、附件与正文间距及右栏展开按钮位置，避免旧样式覆盖新增布局。

- 兼容 DSH 0.1.5-rc.1 / rc.2 的附件草稿，修复建议预填和输入历史读取异常，并保护已有图片与文件。
- 补齐全局面板导航，支持紧凑侧栏、选中状态及返回原会话；从侧栏打开会话时退出全局面板。
