...
- `0.1.5` 与 `0.1.7` 上侧栏能挂上：图标按宿主导出名回退，轮次导航留在会话列内侧，不再滑进侧栏。
- 设置页「打开配置文件」同时等待 `configForms` 和 `settingsScope`，旧宿主不会静默丢掉入口。
- `0.1.7` 侧栏列收到固定 240px，会话和侧栏之间不再空出宿主默认列宽。
- 不再嵌入或改写费用插件。侧栏底部仍显示费用插件自己的入口，点击走它自己的弹窗。
- peer 声明包含 `0.1.7-rc.1`，不再声明 `0.1.6-alpha`。
