...
- 在支持 `sessionTerminal` capability 的 DSH Desktop 中，为当前会话提供工作区终端按钮；普通浏览器与旧版 Desktop 不显示。
...
| 打开工作区终端 | 在支持该能力的 DSH Desktop 中，点击会话标题栏右侧的终端按钮。 |
...
- 本插件只使用 DSH 公开插槽和服务。
- 终端按钮只向公开的 `desktopWindow.openSessionTerminal(sessionId)` 传会话 ID；工作目录由 Desktop Host 解析，不接受路径或命令。
- 不修改宿主源码或会话数据模型。
